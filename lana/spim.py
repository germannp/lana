...
def read_stack(path):
    ...
    assert (
        psutil.virtual_memory()[1] > 1000 * 1024 * 1024
...
def adjust_gamma(stack, gamma=0.5):
    ...
    assert (
        psutil.virtual_memory()[1] > 1.5 * stack.nbytes
...
def stacks2rgb(stack1, stack2, stack3=None):
    ...
    assert (
        psutil.virtual_memory()[1] > 3 * stack1.nbytes
...
def plot_stack(stack, cells=None):
    ...
    assert psutil.virtual_memory()[1] > stack.nbytes, "Not enough memory available"
    img_height = 8
    ...
    slider = Slider(
        slider_ax, "Slice", 0, stack.shape[0] - 1, valinit=mid_slice, valfmt=u"%d"
    )
...
def find_cells(stack):
    """Label spots"""
    assert (
        psutil.virtual_memory()[1] > 8 * stack.nbytes
    ), "Less than 1GiB of memory available"
    ...
    thresholded_stack = thresholded_stack[
        min_idxs[0] : max_idxs[0], min_idxs[1] : max_idxs[1], min_idxs[2] : max_idxs[2]
    ]
    stack = stack[
        min_idxs[0] : max_idxs[0], min_idxs[1] : max_idxs[1], min_idxs[2] : max_idxs[2]
    ]
...
if __name__ == "__main__":
    ...
    DCs["Distance to Closest HEV"] = pd.read_excel(
        ...
        sheetname="Distances_to_closest_HEV",
        header=None,
    )
    DCs["Distance to LN Center"] = pd.read_excel(
        "../Data/SPIM_OutputExample/20k-1_DC-HEV.xls",
        sheetname="Distances_to_Center_of_LN",
        header=None,
    )
    ...
    T_cells["Distance to Closest DC"] = pd.read_excel(
        ...
        sheetname="Distances_between_cell_types",
        header=None,
    )
