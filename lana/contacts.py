...
def _find_by_distance(tracks, dcs, contact_radius, tcz_radius):
    ...
    for time, positions in tracks.sort_values('Time').groupby('Time'):
        ...
        positions = positions[np.linalg.norm(
            positions[['X', 'Y', 'Z']], axis=1) <
...
def simulate_priming(
        tracks, t_cell_ns=(10, 20), dc_ns=(10, 50), min_distances=(0,),
        min_dist_stds=(150 * 0,), contact_radii=(15 / 1.5,),
    ...
    for n_run in range(n_iter):
        for min_dist, min_std, cr, nt, ndc, cond in itertools.product(
                min_distances, min_dist_stds, contact_radii, t_cell_ns, dc_ns,
                conditions):
            cond_tracks = tracks[tracks['Condition'] == cond]
            t_tracks = cond_tracks[cond_tracks['Track_ID'].isin(
                np.random.choice(
                    cond_tracks['Track_ID'].unique(), nt,
                    replace=False))].copy()
            if min_std != 0:
            ...
            r = tcz_radius * (ratio +
                              (1 - ratio) * np.random.rand(ndc))**(1 / 3)
            theta = np.random.rand(ndc) * 2 * np.pi
            phi = np.arccos(2 * np.random.rand(ndc) - 1)
            dcs = pd.DataFrame({
                'X': r * np.sin(theta) * np.sin(phi), 'Y':
                r * np.cos(theta) * np.sin(phi), 'Z': r * np.cos(phi)})
            ...
            if len(min_distances) > 1 or len(min_dist_stds) > 1:
                description.append(
                    'Min. Distance {} +/- {}'.format(min_dist, min_std))
...
def simulate_clustering(
        cd4_tracks, cd8_tracks, cd4_ns=(10,), cd8_ns=(10,), dc_ns=(50,),
        cd8_delays=(0,), contact_radii=(10,), focusing_factors=(1, 2, 4),
        tcz_volume=0.125e9 / 100, n_iter=10):
    ...
    for n_run in range(n_iter):
        for cr, foc_fac, n4, n8, ndc, delay in itertools.product(
                contact_radii, focusing_factors, cd4_ns, cd8_ns, dc_ns,
                cd8_delays):
            ...
            t_tracks = cd4_tracks[cd4_tracks['Track_ID'].isin(
                np.random.choice(
                    cd4_tracks['Track_ID'].unique(), n4, replace=False))]
            ...
            if foc_fac != 1:
                for idx, dc in dcs.iterrows():
                    try:
                        ...
                        dcs.loc[idx, 'Appearance Time'] = dc_contacts[
                            'Time'].min()
                    except KeyError:
                        continue
                ...
                try:
                    run_cd8_pairs = run_cd8_pairs.sort_values(
                        'Time').drop_duplicates('Track_ID')
                except KeyError:
                    pass

            # Check for triples
            run_triples = pd.DataFrame(
            )  # For assertion (and evlt. performance)
            ...
            assert n_triples_of_run <= n_cd8_pairs_of_run, \
            ...
            for _, triple in run_triples.iterrows():
                ...
                cd4_contact_time = triple['Time'] - triple[
                ...
                distance = np.linalg.norm(
                    cd4_position.values - cd8_position.values)
    ...
    return pd.Panel({
        'CD4-DC-Pairs': cd4_pairs, 'CD8-DC-Pairs': cd8_pairs, 'Triples':
        triples})


def plot_details(
        contacts, tracks=None, parameters='Description', context='notebook'):
    ...
    for i, (cond, cond_contacts) in enumerate(contacts.groupby(parameters)):
        ...
        if tracks is not None:
            ...
            for _, contact in cond_contacts.dropna().iterrows():
                ...
                distance = pd.Series(
                    np.linalg.norm(
                        track[['X', 'Y', 'Z']].astype(float) - contact[[
                            'X', 'Y', 'Z']].astype(float), axis=1),
            ...
            distances.index = np.round(
                distances.index, 5)  # Handle non-integer 'Times'
            distats = distances.groupby(distances.index).describe().unstack()
            axes[0].plot(distats.index, distats['50%'], color=color)
            axes[0].fill_between(
                distats.index, distats['25%'], distats['75%'], color=color,
                alpha=0.2)
            axes[0].fill_between(
                distats.index, distats['min'], distats['max'], color=color,
                alpha=0.2)

            sns.distplot(
                durations, bins=np.arange(20 + 1), kde=False, norm_hist=True,
                ax=axes[1], color=color, hist_kws={
                    "histtype": "step", "linewidth": 3, "alpha": 1})
        ...
        ax.scatter(
            ...
            np.linalg.norm(
                cond_contacts[['X', 'Y', 'Z']].astype(np.float64), axis=1),
...
def plot_numbers(
    ...
    detail_ax.set_ylabel(
        'Distribution of T Cells in Contact at {}h'.format(t_detail))
    ...
    for label, _contacts in t_cells_in_contact.groupby(parameters):
        ...
        detail_ax.text(
            i * 2 - 0.5, 0, label, rotation=90, va='bottom', fontsize=size)
        ...
        runs_with_n_contacts = accumulation.apply(
            lambda x: x.value_counts(), axis=1).fillna(0)
        runs_with_n_contacts = runs_with_n_contacts[
            runs_with_n_contacts.columns[::-1]]
        runs_with_geq_n_contacts = runs_with_n_contacts.cumsum(axis=1)
        runs_with_geq_n_contacts.loc[t_max, :] = runs_with_geq_n_contacts.iloc[
            -1]
        detail_runs = runs_with_geq_n_contacts[runs_with_geq_n_contacts.index
                                               <= t_detail * 60]

        for n_contacts in [n for n in runs_with_geq_n_contacts.columns
                           if n > 0]:
            dynamic_ax.fill_between(
                runs_with_geq_n_contacts[n_contacts].index / 60, 0,
...
def plot_percentage(
        contacts, parameters='Description', t_detail=1, n_t_cells=100,
        save=False, palette='deep', context='notebook'):
    """Plot final percentage of T cells in contact with DC"""
    t_cells_in_contact = contacts.drop_duplicates([
        'Track_ID', 'Run', parameters])
    contacts_at_t_detail = t_cells_in_contact[t_cells_in_contact['Time'] <=
                                              t_detail * 60]
    ...
    sorted_contacts = normalized_contacts.reindex_axis(
        sorted(
            total_contacts.columns,
            key=lambda col: total_contacts[col].median()), axis=1)
...
def plot_triples(
        pairs_and_triples, parameters='Description', context='notebook'):
    ...
    _, (activ_ax, triples_ax, timing_ax) = plt.subplots(
        ncols=3, figsize=(12, 5.5))
    ...
    for label, _triples in cd8_activated.groupby(parameters):
        ...
        for n_contacts in [n for n in runs_with_geq_n_contacts.columns
                           if n > 0]:
            percentage = runs_with_geq_n_contacts[n_contacts].iloc[
                -1] / n_runs * 100
            activ_ax.bar(
                i * 2, percentage, color=color,
            ...
            if n_contacts == runs_with_geq_n_contacts.columns.max():
                ...
                next_n = next(
                    n for n in runs_with_geq_n_contacts.columns[::-1]
                    if n > n_contacts)
                next_percentage = runs_with_geq_n_contacts[next_n].iloc[
                    -1] / n_runs * 100
    ...
    for label, _triples in cd8_in_triples.groupby(parameters):
        ...
        sns.distplot(
            _triples['Time Between Contacts'] / 60, kde=False, bins=bins,
...
def plot_triples_vs_pairs(
        triples, parameters='Description', context='notebook'):
    ...
    for run, par in itertools.product(range(int(pairs['Run'].max()) + 1),
        ...
        contact_numbers.loc[max_index, '# CD8 in Triples'] = \
            len(cd8_in_triples)
        cd8_in_pairs = set(
            pairs[(pairs['Run'] == run) & (pairs[parameters] == par)][
                'Track_ID'])
    ...
    for i, (par, numbers) in enumerate(contact_numbers.groupby('Parameter')):
        ...
        axes[0].scatter(
            ...
            color=color)
        ratios = np.arctan(
            numbers['# CD8 in Triples'] / numbers['# CD8 in Pairs'])
        sns.distplot(
            ratios, hist=True, kde=False, color=color, ax=axes[1],
            bins=np.arange(21) * np.pi / 40, hist_kws={
                "histtype": "step", "linewidth": 2, "alpha": 1})
        legend.append(par)
...
def plot_triples_ratio(
    ...
    for run, par in itertools.product(range(int(pairs['Run'].max()) + 1),
        ...
        _licensed = licensed[(licensed['Run'] == run)
                             & (licensed[parameters] == par)]
        _triples = triples[(triples['Run'] == run)
                           & (triples[parameters] == par)]
        # More triples than pairs possible if foc_fac > 1! Thus sets ...
        cd8_in_triples = set(_triples['CD8 Track_ID'])
        n_cd8_in_pairs_or_triples = len(
            cd8_in_triples.union(set(_pairs['Track_ID'])))
        ...
        if n_cd8_in_pairs_or_triples > 0 and n_lic_dcs > 0:
            ...
            n_dcs = int(
                next(
                    sub for sub in cell_numbers.split()[::-1]
                    if sub.isdigit()))
        ...
        if n_cd8_in_pairs_or_triples > 0:
            ...
            ratios.loc[max_index,
                       'CD8 Ratio'] = n_cd8_in_pairs_or_triples / n_CD8
        else:
    ...
    sns.boxplot(
        x='Triple Ratio', y=parameters, data=ratios, notch=False, order=order,
        ax=axes[0])
    sns.stripplot(
        x='Triple Ratio', y=parameters, data=ratios, jitter=True, color='0.3',
        size=1, order=order, ax=axes[0])
    sns.boxplot(
        x='CD8 Ratio', y=parameters, data=ratios, notch=False, order=order,
        ax=axes[1])
    sns.stripplot(
        x='CD8 Ratio', y=parameters, data=ratios, jitter=True, color='0.3',
    ...
    sns.despine(
        ax=axes[1], top=True, right=True, left=True, bottom=False, trim=True)
    plt.tight_layout()
    plt.show()


def plot_situation(
        tracks, n_tracks=6 * 3, n_dcs=50, tcz_volume=0.524e9 / 400,
